...
def pack_rom_data(rooms_dir, data_dir, output_bin, output_inc, verbose=False,
                  scripts_override_dir=None, audio_blob=None):
    ...
    If audio_blob is given (TAD sample-pool groups from pack_groups.py), it
    is placed VERBATIM at appended offset 0. It must stay within the first
    1MB (appended block 0): that block is permanently visible at CPU banks
    $E0-$EF (EXB=4 steady state), which is what the window addresses baked
    into build/audio/tad-groups-data.inc assume. Room/script data shifts up
    behind it; all their offsets flow through rom_data.inc defines.
    """
    data_dir = Path(data_dir)
    override_dir = Path(scripts_override_dir) if scripts_override_dir else None

    audio = Path(audio_blob).read_bytes() if audio_blob else b''
    audio_size = align_to(len(audio), BLOCK_ALIGNMENT)
    if audio_size >= 0x100000:
        logging.error('audio blob %d bytes exceeds appended block 0 (1MB); '
                      'window addressing in tad-groups-data.inc would break',
                      len(audio))
        sys.exit(1)
    ...
    # Section 0: TAD audio group data (must be first — block 0 window)
    # Section 1: Room index + room data
    room_index_offset = audio_size
    room_index_size = room_index_slots * INDEX_ENTRY_SIZE
    room_data_offset = align_to(room_index_offset + room_index_size,
                                BLOCK_ALIGNMENT)
    ...
    # TAD audio group data (offset 0)
    blob[0:len(audio)] = audio
    ...
    if audio:
        logging.info("  TAD audio groups: %d bytes at offset 0 (block 0)",
                     len(audio))
    ...
    inc_lines = [
        ...
        "",
        "; TAD audio group data (window addresses live in tad-groups-data.inc)",
        f".define ROM_DATA_AUDIO_SIZE      ${audio_size:06X}",
        "",
        "; Room data",
...
def main():
    ...
    parser.add_argument('--audio-blob', default=None,
                        help='TAD audio groups blob (pack_groups.py); placed '
                        'at appended offset 0 so it stays in the EXB=4 window')
